...
class Modules
{
     ...
     * @throws \Exception
     */
    public function downloadNew($module_key, $module_license_key, $path)
    {
        ...
        if (isset($request['check-for-updates'])) {
            ...
            try {
                ...
                if (isset($check_updates_result['status']) && $check_updates_result['status'] == "error") {
                    flash($check_updates_result['message'])->warning();
                } else {
                    if (isset($check_updates_result['updates'])) {
                        $remote_updates = $check_updates_result['updates'];
                    } else {
                        $remote_updates = [];
                        flash('Settings::labels.module.up_to_date')->success();
