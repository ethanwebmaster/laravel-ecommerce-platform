...
use Corals\Modules\Ecommerce\Http\Requests\ProductRatingRequest;
...
{
    use DownloadableController, ReviewRateableTrait, EcommerceGallery;
    ...
    public function createRating(ProductRatingRequest $request, Product $product)
    {
        try {
            ...
            $message = ['level' => 'success', 'message' => trans('Ecommerce::labels.product.add_success')];
        ...
        foreach ($gateways as $gateway => $gateway_title) {
            try {
                ...
                if ($Ecommerce->gateway->getGatewayIntegrationId($product)) {
                    ...
                    // $request = $Ecommerce->gateway->FetchProduct(['id' => $Ecommerce->gateway->getGatewayIntegrationId($product)]);
