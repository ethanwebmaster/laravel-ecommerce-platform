...
class CMS
{
     ...
     * @return mixed
     */
    public function getCategoriesList($objects = false, $status = null, $internalState = false)
    {
     ...
     * @return mixed
     */
    public function getTagsList($objects = false, $status = null, $internalState = false)
    {
        $tags = Tag::query();

        $tags = $tags->whereHas('posts', function ($query) use ($internalState) {
            $query->internal($internalState);
        });
